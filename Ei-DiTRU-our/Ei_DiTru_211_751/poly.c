...
void poly_Zw_sub2(const Term* a, const Term *b, Term* r)
{
    /*
    Input:  - a,b: two elements from Z[w]_DN
    Output: - r substracting the terms coefficients-wise and compute the result mod q
    */
...
void poly_Zw_mul(const Term *f, const Term *g, Term *r){
    ...
    /*
    Similar to NTRU multiplication but coefficients are commig from Z[w]
    */
...
void Zw_DN_multiply(Term *f, Term *g, Term *r){
    ...
    /*
    Input:  -f, g: two elements from Z[W]D_N
    Output: - The multiplication result
    The mulitplication is twisted i.e., y^2=-1
    */
    ...
    poly_Zw_mul(f0,g0, mul1); //multiply f0 and g0 and store the result in mul1
    poly_Zw_mul(f1bar, g1, mul2); //multiply f1bar and g1 and store the result in mul2
    poly_Zw_sub2(mul1, mul2, X); //mul1-mul2 //changed for twisting the multiplication

    poly_Zw_mul(f1, g0, mul1 ); 
    poly_Zw_mul(f0bar, g1, mul2);
    poly_Zw_add2(mul1, mul2, Y);
