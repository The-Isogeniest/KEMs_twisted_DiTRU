...
void sample_fixed_type_lf(int *r, const unsigned char u[N2_SAMPLE_BYTES])
{
    int32_t s[N2-1];
    int i;
    ...
    for (i = 0; i<N2_by_3; i++) s[i] |=  1;
        
    crypto_sort_int32(s,N2-1);
...
void sample_lf(Term* r,const unsigned char u[N2_SAMPLE_BYTES+T_SAMPLE_BYTES]){
    ...
    int coeffValues[N2_by_3];

    sample_fixed_type_lf_t(coeffValues,u+N2_SAMPLE_BYTES);
...
void sample_fixed_type_lg(int *r, const unsigned char u[N2_SAMPLE_BYTES]){
    ...
    int leftOut = (N2) - ((N2) / 8) * 8;

    // printf("leftout: %d", leftOut);
    // printf("\n");
    i = (N2)/8;
    ...
    for(int i = 0;i<S;i = i + 3){
        ...
        int randomValue = (u[i]) + (u[i+1]<<8) + (u[2]<<16);
...
void sample_lg(Term* r,const unsigned char u[N2_SAMPLE_BYTES]){
...
void sample_fixed_type_lm(int *r, const unsigned char u[N2_SAMPLE_BYTES])
{
    int32_t s[N2];
    int i;
...
void sample_lm(Term *r, const unsigned char u[N2_SAMPLE_BYTES]){
...
void sample_mask(Term* mask,unsigned char t_seed[LENGTH_OF_HASH]){
    unsigned char uniformbytes[N2_SAMPLE_M_BYTES];
    int filled = 0;

    sha3_512(t_seed,t_seed,LENGTH_OF_HASH);

    while(filled<N2_SAMPLE_M_BYTES){
        int rem = N2_SAMPLE_M_BYTES - filled;
...
void sample_lphi(Term* r,const unsigned char u[N2_SAMPLE_BYTES]){
    sample_lg(r,u);
}  
...
void sample_phi(Term* phi,const Term* h, const Term* m){
    ...
    // printf("inside sample phi \n");

    // print_element(h, N2);
    // print_element(m, N2);
    unsigned char r_seed[2*LENGTH_OF_HASH];

    sha3_512(r_seed,(unsigned char*)(m), N2*2);
    sha3_512(r_seed+LENGTH_OF_HASH,(unsigned char*)(h), N2*2);

    int filled = 0;

    unsigned char uniformbytes[N2_SAMPLE_BYTES];

    sha3_512(r_seed,r_seed,LENGTH_OF_HASH*2);

    while(filled<N2_SAMPLE_BYTES){
        int rem = N2_SAMPLE_BYTES - filled;
        if(rem>=LENGTH_OF_HASH){
    ...
    // printf("uniform bytes \n");

    // for(int i=0;i<N2_by_3; i++) 
    //      printf("%c", uniformbytes[i]);
