...
int ppke_keypair(unsigned char *pk, unsigned char* sk,unsigned char arr_seed[N2_SAMPLE_FG_BYTES]){
    ...
    unsigned long long start_inv, end_inv;
    unsigned long long execution_time_inv = 0;
    ...
    while(flag==0){
        ...
        start_inv = cpucycles_start();
        flag = Almost_inverse_ZpwDN(f,Q,fq);
        end_inv = cpucycles_stop();
        execution_time_inv = (end_inv - start_inv);


        printf("inversion cpu cycles %lld ", execution_time_inv);
