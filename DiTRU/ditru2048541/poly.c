...
int poly_Rq_inv(poly *r, const poly *a)
{
  poly ai2;
  int ret;
  ret = poly_R2_inv(&ai2, a);
...
int poly_Rq_inv_DiTRU(poly *r, const poly *a){
  ...
  poly *c_inv = uu1;
  // printf("The element to be inverted over CN \n");
  // print_polynomial_array(ORDER/2, c);
  int ret = poly_Rq_inv(c_inv,c);
  // printf("The output of the inverse in CN \n");
  // print_polynomial_array(ORDER/2, c_inv);
  poly *b = vv1;
